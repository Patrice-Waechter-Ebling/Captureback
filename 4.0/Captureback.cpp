...
#pragma comment (lib,"webexplorer.lib") 
...
__declspec( dllexport )WNDCLASS wc,wce;
...
unsigned char WindoweCount = 0;
static const TCHAR	ClassName[] = "DDDynaWeb";
...
__declspec( dllexport )void DialogPerso(HWND hDlg);
LRESULT CALLBACK DynaWeb(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
int CALLBACK WinDynaWeb(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow);
int Loop();

extern "C"{
	__declspec( dllexport ) void UnEmbedBrowserObject(HWND hwnd);
	__declspec( dllexport ) void DoPageAction(HWND hwnd, DWORD action);
	__declspec( dllexport ) long DisplayHTMLStr(HWND hwnd, LPCTSTR string);
	__declspec( dllexport ) long DisplayHTMLPage(HWND hwnd, LPTSTR webPageName);
	__declspec( dllexport ) void ResizeBrowser(HWND hwnd, DWORD width, DWORD height);
	__declspec( dllexport ) long EmbedBrowserObject(HWND hwnd);
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
			...
			case IDC_BUTTON1:
				...
				break;
			case IDC_BUTTON2:
					sprintf(buffer,"res://%s/entree.htm",PathFindFileName(__argv[0]));
					OleInitialize(NULL);
						DisplayHTMLPage(0, buffer);
				break;
			case 0xE145:
				SauverFichier("Sauvegarder la capture");
				break;
...
__declspec( dllexport )void SysMenu(HWND hDlg){
	...
	AppendMenu(popmnu,MF_STRING,IDC_BUTTON1,"Visionner une image");
	AppendMenu(popmnu,MF_STRING,IDOK,"Capturer");
	AppendMenu(popmnu,MF_STRING|MF_DISABLED,0xE145,"Sauvegarder la capture");
	bmp=LoadBitmap(wc.hInstance,(LPCTSTR)IDB_BITMAP5);
	SetMenuItemBitmaps(popmnu,IDC_BUTTON1,MF_BYCOMMAND,bmp,bmp);
	bmp=LoadBitmap(wc.hInstance,(LPCTSTR)IDB_CAPTURE);
...
__declspec( dllexport )void DialogPerso(HWND hDlg){
	...
	if(GetOpenFileName(&ofn)==TRUE){
		...
		ReleaseDC(GetDlgItem(hDlg,ID_PREVIEW), GetDC(GetDlgItem(hDlg,ID_PREVIEW)));
		SetLayeredWindowAttributes(hDlg, 0, (255 * 99) / 100, LWA_ALPHA);
	}
}
LRESULT CALLBACK DynaWeb(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	PAINTSTRUCT ps;
	HDC			hdc;
	HWND		g_hWnd=hwnd;
	SetTimer(g_hWnd,IDM_TIMER1,1000,(TIMERPROC) NULL);
	switch (uMsg){
		case WM_SIZE:{
			ResizeBrowser(hwnd, LOWORD(lParam), HIWORD(lParam)-21);
			return(0);
		}
		...
		case WM_DESTROY:{
			...
			--WindoweCount;
			if (!WindoweCount) PostQuitMessage(0);
			return(TRUE);
		}
   		case WM_COMMAND:
			switch (LOWORD(wParam)) {
		...
		case WM_PAINT:
			...
			DrawIcon(hdc,100,100,wce.hIcon);
			RECT rc;
			GetClientRect(g_hWnd,&rc);
			GetLocalTime(&st);
			sprintf(buffer,"Nous sommes %s, le %2d %s %4d",jours[st.wDayOfWeek],st.wDay,mois[st.wMonth-1],st.wYear); //creation du string de date
			SetDlgItemText(g_hWnd,1001,buffer);
			DrawText(hdc,buffer,strlen(buffer),&rc,DT_CENTER|DT_VCENTER);
			EndPaint(g_hWnd,&ps);
			return 0;
		case WM_TIMER:
			GetLocalTime(&st);
			...
			SetDlgItemText(g_hWnd,1001,buffer);
			sprintf(buffer,"%.2d:%.2d:%.2d",st.wHour,st.wMinute,st.wSecond);
			SetDlgItemText(g_hWnd,1002,buffer);
			break;
		case WM_CTLCOLORDLG:
			return (long)wce.hbrBackground;
			break;
		case WM_CTLCOLORSTATIC:
			SetTextColor( (HDC)wParam, RGB(255,0, 255) );
			SetBkMode( (HDC)wParam, TRANSPARENT ); 
...
int CALLBACK WinDynaWeb(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow){
	...
	if (OleInitialize(NULL) == S_OK)	{
		...
		wce.lpfnWndProc = DynaWeb;
		wce.lpszClassName = "DynaWeb";
		wce.hIcon=LoadIcon(hInstance,(LPCTSTR)IDI_ICON2);
		wce.hbrBackground=CreateSolidBrush(RGB(0xFF,0xFF,0xFF));
		wce.hCursor=LoadCursor(hInstance,(LPCTSTR)IDC_CURSOR2);
		RegisterClass(&wce);
		GetClientRect(GetDesktopWindow(),&rc);
		if ((msg.hwnd = CreateWindowEx(0, "Composant DynaWeb (c) Patrice Waechter-Ebling 2002-2005" , wce.lpszClassName, WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZE|WS_MAXIMIZE|WS_THICKFRAME ,0, 0, rc.right, rc.bottom-30,HWND_DESKTOP, NULL, hInstance, 0))){
			HWND hMain=CreateWindowEx(0, "STATIC", lpszCmdLine, WS_VISIBLE|WS_CHILD,1, 1, 700,550,msg.hwnd , (HMENU)45000, wce.hInstance, NULL);
			HWND hCherche=CreateWindow("BUTTON","&Naviger",BS_FLAT|WS_CHILD|WS_VISIBLE ,rc.right-113,2,100,19,msg.hwnd,(HMENU)22000,hInstance,0);
			CreateWindow("STATIC","",WS_CHILD|WS_VISIBLE ,0,2,150,19,msg.hwnd,(NULL),hInstance,0);
			HWND hTexte= CreateWindow("EDIT","Entrez votre requête ici ...",WS_BORDER|BS_FLAT|WS_CHILD|WS_VISIBLE,142,2,rc.right-150-115,19,msg.hwnd,0,wc.hInstance,NULL);
			sprintf(buffer,"res://%s/entree.htm",PathFindFileName(__argv[0]));
			DisplayHTMLPage(msg.hwnd, buffer);
	...
	MessageBox(0, "Échec à l'initialisation OLE32", wce.lpszClassName, MB_OK);
	return(-1);
}
